...
export class WeatherToday {
  ...
  run(finish: (isProcessed?: boolean) => void) {
    const schedules = WeatherToday.getMatchedSchedule(
      this.nowDate,
  ...
  static isValid(accountData: AccountData, nowDate: Date): boolean {
    ...
    return 0 < this.getMatchedSchedule(nowDate, this.scheduleList)?.length
      ? true
      : false;
  }

  private static getSchedules(
    accountData: AccountData
  ): WeatherTodaySchedule[] {
  ...
  private static getMatchedSchedule(
    nowDate: Date,
    scheduleList: WeatherTodaySchedule[]
  ): WeatherTodaySchedule[] {
  ...
  private static getForecastKey(accountData: AccountData): string {
    const key = OdnPlugins.getEnvData(
      WeatherTodayEnvKey.FullName,
      `${WeatherTodayEnvKey.ForecastKey}_${accountData.userId}`
    );
    if (!key) {
  ...
  private tweetWeather(
    schedule: WeatherTodaySchedule,
    cb: (isSuccess: boolean) => void
  ) {
    WeatherToday.forecast.get(
      schedule.location.point,
      (error, weather: WeatherForcast) => {
        if (error) {
